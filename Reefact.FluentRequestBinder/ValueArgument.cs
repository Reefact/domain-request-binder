...
public sealed class ValueArgument<TValue> : Argument
    where TValue : struct {

    #region Statics members declarations

#if !NET8_0
    public static ValueArgument<TValue> UnNamed(TValue? argumentValue) {
    ...
    public ValueArgument(string name, TValue? value) {
    ...
    public ValueArgument<TValue> AppendPrefix(string? prefix) {
        if (prefix == null) { return this; }

        string prefixedName = $"{prefix}.{Name}";

        return new ValueArgument<TValue>(prefixedName, Value);
    }
