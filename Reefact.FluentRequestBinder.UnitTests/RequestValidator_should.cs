...
namespace Reefact.FluentRequestBinder.UnitTests {
    ...
    public class RequestValidator_should {
        ...
        public void test() {
            ...
            RequestConverter<PromoteMember_v1> convert          = Validate.Request(requestWithError);
            // Exercise
            RequiredArgument<AnyId>       teamId      = convert.SimpleProperty(c => c.TeamId).AsRequired(AnyId.From);
            RequiredArgument<string>      utCode      = convert.SimpleProperty(c => c.MemberUtCode).AsRequired();
            RequiredArgument<Temperature> temperature = convert.ComplexProperty(c => c.Temperature).AsRequired(TemperatureConverter.Convert);
            // Verify
            Check.That(convert.ErrorCount).IsEqualTo(1);
            ValidationError error = convert.GetErrors().First();
        ...
        public void test2() {
            ...
            RequiredArgument<Temperature> temperature = convert.ComplexProperty(c => c.Temperature).AsRequired(TemperatureConverter.Convert);
            // Verify
            Check.That(convert.HasError).IsFalse();
            Check.ThatCode(() => {
        ...
        public void test3() {
            ...
            ValidationError utCodeError = convert.GetErrors().First();
